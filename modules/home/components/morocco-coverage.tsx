...
export default function MoroccoCoverage() {
  ...
  return (
    <section className="w-full py-16 bg-white border-y border-[#e4e1ea]/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-sm text-[#54525d] max-w-lg mx-auto mt-2 mb-8">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2.5 max-w-4xl mx-auto">
          {cities.map((city) => (
            <span
              key={city.name}
              className="px-4 py-2 rounded-full bg-[#FAF8FC] border border-[#e4e1ea] text-xs font-bold text-[#19181f] flex items-center gap-1.5 hover:border-[#5906e7] transition-colors cursor-pointer select-none"
            >
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              <span>{city.name}</span>
              {city.districts && (
                <span className="text-[#54525d] font-normal text-[11px]">
                  ({city.districts})
                </span>
              )}
            </span>
          ...
          <button
            ...
            className="px-4 py-2 rounded-full bg-transparent border border-dashed border-[#9e9aa8] text-xs font-semibold text-[#54525d] hover:border-[#19181f] hover:text-[#19181f] transition-colors flex items-center gap-1 cursor-pointer"
          >
            <span>+</span>
        ...
        {suggestCityOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-[#e4e1ea] text-center">
